...
export const schema = {
  limit: z
    ...
    .max(500)
    .default(100)
    .describe("Maximum number of repositories to retrieve"),
  skip: z
    .number()
    .min(0)
    .default(0)
    .describe("Number of repositories to skip (for pagination)"),
};
...
export const handler = withAuth(
  async (params: { limit: number; skip: number }, auth) => {
    ...
    const response = await voneFetch(
      auth,
      `/api/v2/vbr/repositories?limit=${limit}&skip=${skip}`,
    );
