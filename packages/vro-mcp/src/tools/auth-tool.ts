import { z } from "zod";
import { vroAuthRequest } from "../lib/vro-client.js";
...
export const schema = {
  host: z.string().describe("VRO server hostname or IP").optional(),
...
export async function handler(params: {
  ...
  try {
    const host = params.host || process.env.VRO_HOST;
    const username = params.username || process.env.VRO_USERNAME;
    const password = params.password || process.env.VRO_PASSWORD;

    if (!host || !username || !password) {
      const missing: string[] = [];
      if (!host) missing.push("host (VRO_HOST)");
      if (!username) missing.push("username (VRO_USERNAME)");
      if (!password) missing.push("password (VRO_PASSWORD)");
      return toolError(
    ...
    const token = await vroAuthRequest(host, username, password);
    setAuth({ host, token });
